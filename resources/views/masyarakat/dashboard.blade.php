<x-app-layout>
    <div class="flex min-h-screen bg-gray-50">
        <aside class="w-64 bg-white border-r border-gray-200 hidden md:block flex-shrink-0">
            <div class="p-6">
                <h2 class="text-2xl font-bold text-blue-600 tracking-tight">Nickelfox<span
            ...
            <nav class="mt-4 px-4 space-y-2">
                ...
                <a href="{{ route('masyarakat.profile') }}"
                    class="flex items-center px-4 py-3 text-gray-600 hover:bg-gray-50 rounded-xl transition-all">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="pt-10">
                    <a href="{{ route('masyarakat.logout') }}"
                        class="flex items-center px-4 py-3 text-red-600 hover:bg-red-50 rounded-xl transition-all font-medium">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <main class="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 py-10">
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
                <div>
                    ...
                    <p class="text-gray-500">Halo, <span
                            class="font-semibold text-blue-600">{{ session('nama_warga') }}</span>. Sampaikan laporan
                        Anda di sini.</p>
                ...
                <div class="flex items-center space-x-3">
                    <div
                        class="w-11 h-11 bg-blue-600 rounded-2xl flex items-center justify-center text-white font-bold shadow-lg shadow-blue-200">
                        ...
                        <form action="{{ route('laporan.store') }}" method="POST" enctype="multipart/form-data"
                            ...
                            <div>
                                ...
                                <input type="text" name="judul" required
                                    ...
                                    placeholder="Apa yang ingin diadukan?">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Lokasi Kejadian</label>
                                <input type="text" name="lokasi_kejadian" required
                                    class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all outline-none text-sm"
                                    placeholder="Contoh: Depan Toko Maju, RT 02...">
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input type="file" name="foto"
                                    class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-600 hover:file:bg-blue-100 cursor-pointer">
                            </div>
                            <button type="submit"
                                class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-blue-100 uppercase tracking-wide">
                                Kirim Aduan
                            </button>
                        </form>
                    </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left border-collapse">
                                ...
                                <tbody class="divide-y divide-gray-50 text-sm">
                                    ...
                                    <tr class="hover:bg-gray-50/50 transition-colors">
                                        <td class="px-6 py-4 text-center">
                                            <div class="flex flex-col">
                                                <span
                                                    class="text-gray-900 font-bold text-xs">{{ $item->created_at->format('d M Y') }}</span>
                                                <span
                                                    class="text-blue-500 font-black text-[10px]">{{ \Carbon\Carbon::parse($item->created_at)->timezone('Asia/Jakarta')->format('H:i') }}
                                                    WIB</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                            ...
                                            <div
                                                class="text-[10px] text-blue-600 font-bold mb-1 uppercase tracking-wider">
                                                📍 {{ $item->lokasi_kejadian }}</div>
                                            <div class="text-xs text-gray-400 truncate max-w-[200px] italic">
                                                "{{ $item->isi_laporan }}"</div>
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            @if($item->status == 'pending')
                                            <span
                                                class="px-3 py-1 rounded-full text-[10px] font-black bg-amber-50 text-amber-600 border border-amber-100 uppercase">Menunggu</span>
                                            @elseif($item->status == 'proses')
                                            <span
                                                class="px-3 py-1 rounded-full text-[10px] font-black bg-blue-50 text-blue-600 border border-blue-100 uppercase">Diproses</span>
                                            @elseif($item->status == 'selesai')
                                            <span
                                                class="px-3 py-1 rounded-full text-[10px] font-black bg-green-50 text-green-600 border border-green-100 uppercase">Selesai</span>
                                            @else
                                            <span
                                                class="px-3 py-1 rounded-full text-[10px] font-black bg-red-50 text-red-600 border border-red-100 uppercase">Ditolak</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 text-center">
                                            <button
                                    ...
                                    <tr>
                                        <td colspan="4"
                                            class="px-6 py-20 text-center text-gray-400 text-sm italic font-medium">
                                            Belum ada aktivitas laporan.</td>
                                    </tr>
                                    @endforelse
                                </tbody>
                            </table>
